...
class MY_Model extends CI_Model {
	 ...
	 * $input : mang cac du lieu dau vao
	 */
	function find_duplicate($field = '',$time = array(),$month = false)
	{
		$this->db->select($field);
		if ($month === true) {
			$this->db->group_by("DATE_FORMAT(created,'%Y-%m')");
			$this->db->where($time);
			$query = $this->db->get($this->table);
			return $query->result();
		}else{
			$this->db->group_by("DATE_FORMAT(created,'%Y')");
			if ($time) {
				$this->db->where($time);
				$query = $this->db->get($this->table);
				return $query->row();
	...
	function statistical($input = array(),$time = array() ,$field_sum='')
	{
		$this->db->select('created');
		$this->db->select($field_sum);
		$this->get_list_set_input($input);
		$this->db->where($time);
		$query = $this->db->get($this->table);
		return $query->result();
