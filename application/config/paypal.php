...
if ($this->CI->session->userdata('data_payment')) {
	$data = $this->CI->session->userdata('data_payment');
	/**
	* Sandbox / Test Mode
	* -------------------
	*
	...
	*/
	 ...
	 */
	$config['APIUsername'] = $data->username ;//$config['Sandbox'] ? 'haongodev_api1.gmail.com' : 'PRODUCTION_USERNAME_GOGES_HERE';
	$config['APIPassword'] = $data->password ;//$config['Sandbox'] ? 'G6MSC9MM636HV8B8' : 'PRODUCTION_PASSWORD_GOGES_HERE';
	$config['APISignature'] = $data->signature ;//$config['Sandbox'] ? 'AUL5SpfcA-iN.9TnYLATDlEHkkKOAOcY7Oqtoi3YW9ArKle1jBnUm8Xa' : 'PRODUCTION_SIGNATURE_GOGES_HERE';
}
